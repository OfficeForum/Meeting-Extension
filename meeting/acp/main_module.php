...
class main_module
{
	...
	function main($id, $mode)
	{
		...
		$mode			= $request->variable('mode', 'manage');
		$delete			= $request->variable('delete', '');

		$start			= $request->variable('start', 0);

		// Define the ext path
		$ext_path					= $phpbb_extension_manager->get_extension_path('oxpus/meeting', true);
		$phpbb_path_helper			= $phpbb_container->get('path_helper');
		$ext_path_web				= $phpbb_path_helper->update_web_root_path($ext_path);
		$ext_path_images			= $ext_path_web . 'includes/images/';
		$ext_path_theme				= $ext_path_web . 'styles/' . $user->style['style_path'] . '/theme';
		...
		$template->assign_vars(array(
			...
			'S_MEETING_JS'		=> (in_array($mode, array('add', 'manage'))) ? true : false,
		));
		...
		$closed					= $request->variable('closed', 4);
		$e_date					= $request->variable('e_date', '');
		$e_time					= $request->variable('e_date_time', '');
		$filter					= $request->variable('filter', '');
		$filter_by				= $request->variable('filter_by', 'none');
		$group_id				= $request->variable('group_id', array(0 => ''));
		$group_id_2				= $request->variable('group_id_2', array(0 => ''));
		...
		$meeting_link			= $request->variable('meeting_link', '', true);
		$meeting_location		= $request->variable('meeting_location', '', true);
		$meeting_notify			= $request->variable('meeting_notify', 0);
		$meeting_places			= $request->variable('meeting_places', 0);
		$meeting_recure_value	= $request->variable('meeting_recure_value', 0);
		$meeting_start_value	= $request->variable('meeting_start_value', 0);
		$meeting_subject		= $request->variable('meeting_subject', '', true);
		$sort_field				= $request->variable('sort_field', 'meeting_time');
		$sort_order				= $request->variable('sort_order', 'ASC');
		$start					= $request->variable('start', 0);
		...
		$basic_link_smode = $this->u_action;
		...
		include($ext_path . 'includes/functions.' . $phpEx);
		...
		switch($mode)
		{
			...
			case 'manage':
				...
				if ($submit)
				{
					...
					$message = $meeting_save_text . adm_back_link($basic_link . '&amp;mode=manage');
					trigger_error($message);
				}
				...
				if ($m_id || $mode == 'add')
				{
					...
					// Preset time fields
					$m_date		= meeting_date_edit('m_date', (int) $meeting_time, $user);
					$e_date		= meeting_date_edit('e_date', (int) $meeting_end, $user);
					$u_date		= meeting_date_edit('u_date', (int) $meeting_until, $user);
					...
					if ($total_meeting)
					{
						...
						if ($total_meetings > $per_page)
						{
							...
							$template->assign_vars(array(
								...
								'TOTAL_MEETINGS'	=> $user->lang('MEETING_TOTALS', $total_meetings),
							));
						}
					}
					else
